...
from pycls.datasets.rotnet import RotNetDataset
from pycls.ssl.evaluator import SSLEvaluator
...
def main(cfg):
    ...
    logger.info("Rotation Dataset {} Loaded Sucessfully. Total Train Size: {}\n".format(cfg.DATASET.NAME, train_size))

    trainSet_path = data_obj.saveSet(setArray=trainSet, setName='trainSet', save_dir=cfg.EXP_DIR)
    ...
    if cfg.MODEL.TYPE == 'linear':
        model = SSLEvaluator(n_input=cfg.MODEL.NUM_INPUT, n_classes=cfg.MODEL.NUM_OUTPUT, n_hidden=None)
    else:
        model = SSLEvaluator(n_input=cfg.MODEL.NUM_INPUT, n_classes=cfg.MODEL.NUM_OUTPUT, n_hidden=cfg.MODEL.NUM_HIDDEN)
    print("Evaluation model: {}\n".format(cfg.MODEL.EVAL))
    logger.info("Evalution model: {}\n".format(cfg.MODEL.EVAL))

    # Initialize the SSL model
    ssl_model = model_builder.build_model(cfg)
    ssl_checkpoint_file = os.path.join(os.path.abspath('..'), cfg.TEST.MODEL_PATH)
    ...
    test_acc = test_model(trainSet_loader, eval_checkpoint_file, ssl_checkpoint_file, cfg, cur_episode=1)
...
def train_model(train_loader, val_loader, model, ssl_model, optimizer, cfg):
    ...
    for cur_epoch in range(start_epoch, cfg.OPTIM.MAX_EPOCH):
        # Train for one epoch
        train_loss, clf_iter_count = train_epoch(train_loader, model, ssl_model, loss_fun, optimizer, train_meter, \
        ...
        save_plot_values([plot_epoch_xvalues, plot_epoch_yvalues, plot_it_x_values, plot_it_y_values],\
            ["plot_epoch_xvalues", "plot_epoch_yvalues", "plot_it_x_values", "plot_it_y_values"], out_dir=cfg.EPISODE_DIR, isDebug=False)
        ...
        save_plot_values([plot_epoch_xvalues, plot_epoch_yvalues, plot_it_x_values, plot_it_y_values], \
                ["plot_epoch_xvalues", "plot_epoch_yvalues", "plot_it_x_values", "plot_it_y_values"], out_dir=cfg.EPISODE_DIR)
    ...
    checkpoint_file = cu.save_checkpoint(info="final_eval_", \
        model_state=best_model_state, optimizer_state=best_opt_state, epoch=100, cfg=cfg)
    ...
    return None, None, checkpoint_file


@torch.no_grad()
def test_model(test_loader, eval_checkpoint_file, ssl_checkpoint_file, cfg, cur_episode):
...
def train_epoch(train_loader, model, ssl_model, loss_fun, optimizer, train_meter, cur_epoch, cfg, clf_iter_count, clf_change_lr_iter, clf_max_iter):
